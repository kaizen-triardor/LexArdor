...
def get_citation_graph(slug: str, depth: int = 1,
                       user: dict = Depends(get_current_user)):
    """Citation graph for vis.js — nodes (articles) and edges (citations)."""
    from db.legal_schema import get_legal_db

    conn = get_legal_db()

    # Get all articles for this law
    doc = conn.execute("SELECT id, title FROM legal_documents WHERE slug=?", (slug,)).fetchone()
    if not doc:
        conn.close()
        raise HTTPException(status_code=404, detail="Zakon nije pronađen")

    articles = conn.execute(
        "SELECT id, article_number, full_text FROM legal_articles WHERE document_id=?",
        (doc["id"],)
    ).fetchall()
    ...
    edges = []
    node_ids = set()
    law_groups = {slug: doc["title"]}

    # Add nodes for this law's articles
    for a in articles:
        node_id = f"{slug}_clan_{a['article_number']}"
        nodes.append({
            "id": node_id,
            "label": f"Čl. {a['article_number']}",
            "group": slug,
            "title": (a["full_text"] or "")[:150],
        })
    ...
    for a in articles:
        citations = conn.execute(
            "SELECT target_document_slug, target_article_number, citation_text FROM citation_edges WHERE source_article_id=?",
            (a["id"],)
        ).fetchall()
        for c in citations:
            target_slug = c["target_document_slug"] or slug
            target_id = f"{target_slug}_clan_{c['target_article_number']}"
            from_id = f"{slug}_clan_{a['article_number']}"

            edges.append({
                "from": from_id,
                "to": target_id,
                "label": (c["citation_text"] or "")[:30],
            })

            # Add target node if not already present
            if target_id not in node_ids:
                node_ids.add(target_id)
                # Look up the target law name
                if target_slug not in law_groups:
                    tdoc = conn.execute("SELECT title FROM legal_documents WHERE slug=?",
                                         (target_slug,)).fetchone()
                    law_groups[target_slug] = tdoc["title"] if tdoc else target_slug
                nodes.append({
                    "id": target_id,
                    "label": f"Čl. {c['target_article_number']}",
                    "group": target_slug,
                    "title": law_groups[target_slug],
                })

    # Get incoming citations (depth >= 1)
    article_ids = [a["id"] for a in articles]
    if article_ids:
        placeholders = ",".join("?" * len(article_ids))
        # Find articles that cite our articles
        for a in articles:
            inbound = conn.execute("""
                SELECT ce.source_article_id, ce.citation_text, la.article_number, ld.slug, ld.title
                FROM citation_edges ce
                JOIN legal_articles la ON ce.source_article_id = la.id
                JOIN legal_documents ld ON la.document_id = ld.id
                WHERE ce.target_document_slug = ? AND ce.target_article_number = ?
            """, (slug, a["article_number"])).fetchall()
            for ib in inbound:
                src_id = f"{ib['slug']}_clan_{ib['article_number']}"
                tgt_id = f"{slug}_clan_{a['article_number']}"
                edges.append({"from": src_id, "to": tgt_id, "label": ""})
                if src_id not in node_ids:
                    node_ids.add(src_id)
                    if ib["slug"] not in law_groups:
                        law_groups[ib["slug"]] = ib["title"]
                    nodes.append({
                        "id": src_id,
                        "label": f"Čl. {ib['article_number']}",
                        "group": ib["slug"],
                        "title": ib["title"],
                    })
